    ...
    var StorageWidget = {
        ...
        init:function(){
            ...
            if(window.localStorage){  //使用localStorage存储
	              ...
	              }catch(e){showSystemInfo(MOZ_INFO.SECRET_MODE);}
    ...
    var MozillaTool = (function(){
      ...
      function loadScrollResource(){
        ...
        function setHomePage(btn){
            ...
            if (document.all){ //IE浏览器-注意此处url必须为完整的地址，包含协议，否则不生效
                try{
                ...
                if(isFirefox){//Firefox浏览器
                    if(NEW_EXTENSION){ //新扩展
        ...
        function isBrowser(type){
            var browser = "",
                fbrowser=isFBrowser();
            if(isWBrowser()){ //可能为IE或webkit内核封装浏览器
        ...
        function isWBrowser(){
              ...
              for(;wst<wlen;wst++){
                  ...
                  if(UA.indexOf(temp) > -1){flag = true;break;}
              }
              return flag;
        }
        //主流浏览器判定
        ...
        function is360(){
            var browser = "";
             if(UA.indexOf("360SE") > -1){//360旧版IE内核浏览器
                 browser = "360IE";
             }else{
                 var browser = ""; //安全浏览器（chrome内核）
                 if(is360SE()){//低版本的360安全浏览器（chrome内核）
                    browser = "360SE";
                 }else{
                    ...
                    if(isTrack && isScope && isV8Locale){
                        browser = "360SE"; 
                    }else if(isTrack && isScope && !isV8Locale){//极速浏览器（chrome内核）
                        browser = "360JS";
                    }
                 }
             }
        ...
        function is360SE(){ //判断为低端版本的360安全浏览器（chrome内核）
        ...
        scroll: function(){
            ...
            if(sideReferEle.length && sideTargetEle.length && footerEle.length){
                var sTop = MozillaTool.getScrollPos().top;
                    ...
                    sideReferHeight = sideReferEle.height(),
                    sideTargetHeight = sideTargetEle.height(),
                    footerHeight = footerEle.height(),
                    sideReferTop = sideReferEle.get(0).getBoundingClientRect().top;
                    footerTop = footerEle.get(0).getBoundingClientRect().top,
                    pageMinus = pageVisHeight-sideReferHeight;
                var sInfo;
                switch(barType){
                    case "relative":
                       sInfo = {
 ...
 */
(function(){
	 var MarqueeWidget = function(options){
          var that = this;
          that.settings = {
             	 wrapper: null,       //容器-需要切换的元素为其直接子元素
             	 speed: 10000         //切换的速度
          };
          that.settings = $.extend(that.settings,options);
          that.init();
	 }
	 MarqueeWidget.prototype = {
    	 	init:function(){
    	 		var that = this,
    	 		    settings = that.settings,
    	 		    wrapper = settings.wrapper,
    	 		    wheight = wrapper.height;
    	 		$.each(wrapper,function(i,n){
    	 			var tarEle = $(n),
    	 			    list = tarEle.children();
    		        if(list.length){
    		        	setInterval(function(){
    		               n.appendChild(tarEle.children().get(0));
    		        	},settings.speed);
    		        }
    	 		});
    	 	}
	 };
	 MarqueeWidget.build = function(options){
          new MarqueeWidget(options);
	 };
	 if(!window.MarqueeWidget) window.MarqueeWidget = MarqueeWidget;
})();
...
(function(){
    var TraceWidget = (function(){
        ...
        function restoreDefaultAction(btn,href){
            ...
            switch(tagName){
                case "form":  btn.submit();break;
                default:      window.top.href = href;break;
            }
        }
        //获取频道id
        ...
        function getChannelid(){
        	   // alert("NEW_EXTENSION="+NEW_EXTENSION+"  NEW_EXTENSION_50="+NEW_EXTENSION_50);
                ...
                } else if (window.cehomepage || window.mozCNUtils) { // 兼容之前的扩展
                    try{
                            if (window.cehomepage) {
                                var startup = window.cehomepage.startup;
                                if ( !startup.channelid ) {
        ...
        SearchEngine.prototype = {
            ...
            bindEvents:function(){
                ...
                keyEle.click(function(evt){
                   ...
                   if(!val && that.showAuto && that.showHotBtn){//在文本为空，且设置为自动显示，且为可显示热门推荐搜索的区块下
                      //2013-12-31 start
                      that.initiated = true;
                      //2013-12-31 end
                ...
                $(tipKeySelector).keyup(function(evt){
                    ...
                    if(keyCode == 38){
                        ...
                        searchTips.find("li:eq("+changeIndex+")").addClass(tipSelectClass).siblings().removeClass(tipSelectClass);
                    ...
                    //Down
                        ...
                        if(!val && that.showAuto && that.showHotBtn && that.initiated){ //显示热门推荐搜索
            ...
            init:function(){
                ...
                //初始化自动热词自动显示
                switch(hotAuto){
                        case 1: that.showAuto = true;hotListEle.find(".engine-hot-set input").get(0).checked = true;break;
                }    
                that.bindEvents();
            ...
            showSearchNav:function(){
                ...
                $.each(tabs,function(i,n){ 
                    if(n.name == "web" && !firstTab){showHotBtn = true;}
...
(function(){
 //modified by winter start
    //节日皮肤
    //推荐皮肤-包括节日、新主题等，如果没有则为空数组
    var SKIN_RECOMMEND = [
        /*{ 
           img: "img/thumbs/winter.jpg",
           name: "温暖过冬",
           type: "winter",
           recommend:true,
           holiday: false
    ...
    var SKIN_DEFAULT = [
        ...
        /*,
        { 
          img: "img/skin/thumbs/color.jpg",
          name: "简约素色",
          title:"春意盎然",
          type: "green",
          classname: "skin-color",
          optionList: [
              {type:"orange",name:"夏日橙色",classname:"color-orange"},
              {type:"yellow",name:"清新淡黄",classname:"color-yellow"},
              {type:"green",name:"春意盎然",classname:"color-green"},
              {type:"dblue",name:"神秘深蓝",classname:"color-dblue"},
              {type:"purple",name:"性感紫色",classname:"color-purple"}
          ]
        }*/
    ];
    //其他皮肤列表
	...
	/*
		...
		*/
        ...
        /*,
        { 
           img: "img/thumbs/worldcup.jpg",
           name: "激情世界杯",
           type: "worldcup"
        }*/
       
    ]; 
    var SKIN_LIST_TOTAL = SKIN_DEFAULT.concat(SKIN_RECOMMEND,SKIN_EXTRA);
    ...
    SkinWidget.prototype = {
        ...
        bindEvents: function(){
            ...
            SlideWidget.build({
              	...
              	nextEle: nextEle,
              	prevEle: prevEle,
              	success:function(cPage,tPage){//滑动完毕后回调函数设置当前页和总的页数
        ...
        init: function(){
            ...
            //modified by winter start
            var defaultData = StorageWidget.getItem("n_2014_skin",null),
                recommend = SKIN_RECOMMEND.length ? SKIN_RECOMMEND[0] : null;
            //节日主题则默认皮肤强制更改
            if(recommend && recommend.holiday && (!defaultData || (defaultData && defaultData.type == "blue"))){
                defaultData = {title:recommend.name,type:recommend.type,url:recommend.type};
                skinData = defaultData;
            }
            //加载皮肤列表
        ...
        initSkinStyle: function(skinData,loadFlag){
            ...
            if(!skinData){
                var defaultSkin = SKIN_DEFAULT[0];
                skinData = {type:defaultSkin.type,title:defaultSkin.title,url:defaultSkin.type};
            }
            //添加日志(页面加载时进行)
        ...
        loadSkinList: function(){
            var that = this,
                ...
                start = 0,
                len = SKIN_LIST_TOTAL.length;
            cPageEle.html(1);
            tPageEle.html(Math.ceil(len/pageSize));
            for(;start<len;start++){
                var temp = SKIN_LIST_TOTAL[start],
                    cls = temp.classname,
                ...
                if(optList && optList.length){
                    ...
                    $.each(optList,function(i,n){
                        ...
                        if(skin == n.type) {optionSelected = true;liSelected = true;}
                ...
                htm += "<li data-title='"+(temp.title ? temp.title : temp.name)+"' class='btn-color"+(isLiFirst ? " first ": "")+(cls ? " "+cls : "")+(liSelected ? " selected" : "")+"' data-type='"+temp.type+"'>"+
                       "   <div class='skin-img'>"+
                       "<img class='img-load-delay' src='' data-url='"+temp.img+"'/>"+optHtm+authorStr+"</div>"+
                       "      <div class='skin-name'>"+
                       "       <span class='moz-radio'>"+temp.name+(temp.recommend ? "<sup>新</sup>" : "")+"</span></div>"+
                       "</li>";
        ...
        changeSkin:function(btn,skin){
            ...
            //节日添加标注
            if(SKIN_RECOMMEND.length && skin!= SKIN_RECOMMEND[0].type){
                panelMenuEle.addClass("holiday").find("em").html(SKIN_RECOMMEND[0].name); 
            }else{
              ...
              if(coupletEle.length){
                  if(coupletTimeVersion!= coupletVersion){
                      ...
                      if(coupletImgs.length){
                          ...
                          if(show == 1){coupletEle.show();}else{coupletEle.hide();}
                      }
                  }
              }
        },
...
(function(){
	                  ...
	                  if(menuLen>1){
	                          ...
	                          for(var i=0; i<menuLen;i++){
	                            var menuLi = menuEles.eq(i),
	                                ...
	                                perVal = 0;
	                            if(perAttr){
	                                perVal = parseFloat(perAttr),
	                                temp = perTotal + perVal;
	                                if(temp>1){
                      ...
                      tabMenu.attr("minterval",menuInterval);
	                    ...
	                    if(tabDetail.length && autoFlag){
	                        ...
	                        tabDetail.mouseleave(function(evt){
	                        	...
	                        	if(tabMenu.attr("minterval") == undefined){btn.off("mouseleave mouseover mouseenter");return;}  
	                        });
	                    	...
	                    	var overInterval =  setInterval(function(){
	                            ...
	                            if(mInterval != undefined){
		                            ...
		                            if(beforeTime){
		                            	...
		                            	if(minTime > MOZ_INFO.OVER_TIME){ //超过时间则取消自动切换
        ...
        //加载旅游版块的内容
        TabWidget.loadTour = function(btn,forEle,callback){
            loadTourTab(btn,forEle,callback);
        };
        ...
        function clearTabInterval(type){
            var itArr = [];
            switch(type){
                case "menu":itArr = TAB_MENU_ARR; TAB_MENU_ARR=[];break;
                case "cont":itArr = TAB_CONT_ARR; TAB_CONT_ARR=[];break;
            }
            if(itArr.length){
                var start = 0,
                    len = itArr.length;
        ...
        //显示旅游的信息 - not used
        function loadTourTab(btn,wrapper,callback){
          	$.ajax({
              		url: "http://open.tuniu.com/distributor/product/firefox-new/index2.php?t="+new Date().getTime(),
              		type: "get",
              		dataType: "jsonp",
                    timeout:MOZ_INFO.TIME_OUT,
              		error:function(){
                       wrapper.html("<div class='error'>"+MOZ_INFO.SYSTEM_IS_BUSY+"</div>");
              		},
              		success:function(data){
                        var status = data ? data.status : 0,
                            htm = MOZ_INFO.SYSTEM_IS_BUSY;
                        if(status == 1){
                            btn.length && btn.attr("data-loaded",1);
                            htm = tourTemp(data);
                            wrapper.html(htm);
        ...
        //旅游模板  not used
        function tourTemp(data){
            var htm = "";
            if(data){
                var fimg1 = data.position1,
                    fimg2 = data.position2,
                    newslist = data.position3,
                    imglist1 = data.position4,
                    imglist2 = data.position5,
                    imgSize = 2,
                    linkSize = 6;
                 //顶部内容
                htm = "<div class='hot-mod hot-top'><div class='hot-left'><ul>";
                //焦点图
                if(fimg1 && fimg1.items){
                  var fitem = fimg1.items;
                  htm +=  " <li class='hot-img-one'>"+
                       "   <div>"+
                       "       <a class='news-img-wrapper' href='"+fitem.url+"' title='"+fitem.title+"'>"+
                       "         <img src='"+fitem.imgurl+"'/>"+
                       "       </a>"+
                       "       <p class='img-text'><a href='"+fitem.url+"' title='"+fitem.title+"'>"+fitem.title+"</a></p>"+
                       "    </div>"+
                       "</li>";
                }
                //焦点小图
                htm += "<li class='hot-img-two'>";
                if(fimg2 && fimg2.items.length){
                    $.each(fimg2.items,function(i,n){
                      if(i<imgSize){
                     	 htm +=  " <div class='"+(i==0 ? "img-two-left" : "img-two-right")+"'>  "+      
	                          "    <a class='news-img-wrapper' href='"+n.url+"' title='"+n.title+"'>"+
	                          "       <img src='"+n.imgurl+"'/>"+
	                          "    </a>"+
	                          "    <p class='img-text'><a href='"+n.url+"' title='"+n.title+"'>"+n.title+"</a></p>"+
	                          " </div>";
                      }
                    });
                }
                htm += "</li>";
                htm+= "</ul></div><div class='hot-right'>";
                //右侧列表
                htm+=" <ul class='item-list right-top'>";
                if(newslist && newslist.items){
                   $.each(newslist.items,function(i,n){
                   	    if(i<2*linkSize){
	                        if(i == linkSize){
	                          htm += "</ul><ul class='item-list'>";
	                        }
	                        htm+= " <li"+(i==0 ? " class='first'" : "")+"><a href='"+n.url+"' title='"+n.title+"'>"+n.title+"</a></li>";
                        }
                   });
                }
                htm+=" </ul></div></div>";
                //底部内容
                htm += "<div class='hot-mod hot-bottom'><div class='hot-left'>"+imgListTemp(imglist1.items)+"</div>";   
                htm +="<div class='hot-right'>"+imgListTemp(imglist2.items)+"</div>";
                //右侧列表
                htm +="</div></div>";
            }
            return htm;
        ...
        //底部图片新闻列表模板 not used
        function imgListTemp(list){
        	var htm = "",
        	    size = 6;
        	if(list && list.length){
		            var imgItem = list[0];
		       		htm +=    "<div class='hot-mod-inner hot-bottom-img'>"+
		                      "<a class='news-img-wrapper' href='"+imgItem.url+"' title='"+imgItem.title+"'><img src='"+imgItem.imgurl+"'/></a>"+
		                      "<div class='hot-bottom-imginfo'>"+
		                      "    <a href='"+imgItem.url+"' class='first bom-img-text' title='"+imgItem.title+"'>"+imgItem.title+"</a>"+
		                      "    <p><a href='"+imgItem.url+"' title='"+(imgItem.intro ? imgItem.intro : imgItem.title)+"'>"+(imgItem.intro ? imgItem.intro : imgItem.title)+"</a></p>"+
		                      "</div>"+
		                      "</div>";
		            htm += "<div class='hot-mod-inner hot-bottom-list'>"+
		                   "<ul class='item-list hot-list'>";
		            $.each(list,function(i,n){
		            	if(i>0 && i<=size){
		            		htm +=  "<li><a href='"+n.url+"' title='"+n.title+"'>"+n.title+"</a></li>";	
		            	}
		            });
		            htm += "</ul></div>";
            }
    ...
    SetWidget.prototype = {
        ...
        verSwitch: function(ver,initFlag){
           ...
           // verRelatedCSS.attr("href", verData["cssurl"] );  
            ...
            //处理设置里的宽屏的radio显示
            initFlag && verEle.find(".opt-radio[data-type='"+ver+"']").addClass("selected").siblings().removeClass("selected");
        },
        ...
        recSwitch: function(rec,initFlag){
            ...
            that.recData = rec;
            switch(rec){
                case "h": recRelatedEle.hide();break;
                default:  recRelatedEle.show();
            }
            initFlag && recEle.find(".opt-radio[data-type='"+rec+"']").addClass("selected").siblings().removeClass("selected");
        },
        ...
        flipSwitch: function(flip,initFlag){
            ...
            that.flipData = flip;
            switch(flip){
                 case "h": flipRelatedEle.hide();flipRelatedEle.removeAttr("flip");break;
                 default: //flipRelatedEle.show();
                           flipRelatedEle.attr("flip","flip");
            }
            initFlag && flipEle.find(".opt-radio[data-type='"+flip+"']").addClass("selected").siblings().removeClass("selected");
...
(function(){
    ...
    var WEATHER_SETTINGS = {
            W_SET_ELE  :  null,               //天气设置元素
            W_SET_SIGN_ELE : null,            //星座设置元素
            W_WRAP_ELE  :  null,              //天气显示元素
            W_WRAP_SIGN_ELE : null,           //星座wrapper元素
            W_BRIEF_ELE  :  null,             //天气header简单显示
            W_BRIEF_SIGN_ELE: null,           //星座简单显示

            W_DEFAULT_CID: "CHXX0008",        //天气默认加载的cityid 写死 避免多加载
            W_DEFAULT_CNAME: "北京",          //天气默认加载的cityname

            W_INIT_FLAG  :  false,            //地理位置是否需要重新初始化获取
            W_LOC_FLAG  : false,              //是否为初始地理定位(需要还原设置下拉框的初始选项)
            W_SIGN_INIT_FLAG  :  false,       //星座内容是否初始化

            W_CITY_ID  :  null,               //保存确认成功获取后的cityid
            W_CITY_NAME : null,               //保存确认成功获取后的cityname
            W_SIGN_ID  :  null,               //保存用户设置的星座signid
            W_CONFIRM_CID  :  null,           //保存确认操作后的cityid 
            W_CONFIRM_CNAME  :  null,         //保存确认操作后的cityname
            W_SIGN_LIST  :  [],               //保存星座列表  
            W_SAVE_INFO  :  null,             //保存天气信息（8小时更新一次）

            //星座样式表
            W_SIGN_CLS  :  ["baiyang","jingniu","shuangzi","juxie","shizi","chunv","tianping","tianxie","sheshou","mojie","shuiping","shuangyu"],
            W_CHECK_INTERVAL  :  10*60*1000 //重新加载天气间隔
    };
    //天气、星座相关url
    var WEATHER_lINKS = {
        //获取星座
        //SIGN_FETCH_LINK: "res/xingzuo.js",
        //SIGN_FETCH_LINK: "https://tiny.lishitu.com/open/changeXinzuo",
        SIGN_FETCH_LINK: "https://tiny.51hl.me/open/changeXinzuo",
        //获取城市列表
        //CITY_FETCH_LINK:"https://platform.sina.com.cn/open_weather/area_list?level=4&app_key=2278817810",
        //PROV_FETCH_LINK: "res/areas_new.json",
        PROV_FETCH_LINK: "res/areas.js",
    };
    ...
    WeatherWidget.build = function(options){
        if(!options)return;
        WeatherWidget.settings = options;

        //天气相关设置
        WEATHER_SETTINGS.W_SET_ELE = options.setEle;
        WEATHER_SETTINGS.W_BRIEF_ELE = options.briefEle;
        WEATHER_SETTINGS.W_BRIEF_SIGN_ELE = options.signShowEle;
        WEATHER_SETTINGS.W_WRAP_ELE = options.wrapper;
        WEATHER_SETTINGS.W_SET_SIGN_ELE = options.signSetEle;
        WEATHER_SETTINGS.W_WRAP_SIGN_ELE = options.signWrapper;

        var saveInfo = StorageWidget.getItem("n_2014_w_info",null);
        if(saveInfo){
        	saveInfo.cityid && (WEATHER_SETTINGS.W_CITY_ID = saveInfo.cityid);
        	saveInfo.cityname && (WEATHER_SETTINGS.W_CITY_NAME = saveInfo.cityname);
       		 WEATHER_SETTINGS.W_SAVE_INFO = saveInfo;
        }else{
        	WEATHER_SETTINGS.W_CITY_ID = WEATHER_SETTINGS.W_DEFAULT_CID;
        	WEATHER_SETTINGS.W_CITY_NAME = WEATHER_SETTINGS.W_DEFAULT_CNAME;
        }
        
        //加载省份列表  true-初始化标识 
        //loadProvince(true,function(){
	        //加载天气  （有本地存储则为本地存储城市  否则默认均为北京）
	       // alert(WEATHER_SETTINGS.W_CITY_ID+"  "+WEATHER_SETTINGS.W_CITY_NAME);
	    loadWeather(WEATHER_SETTINGS.W_CITY_ID,WEATHER_SETTINGS.W_CITY_NAME);

       //}); 

        
        //===========================星座相关=====================
        WEATHER_SETTINGS.W_SIGN_ID = StorageWidget.getItem("n_2014_w_signid",0);
        WEATHER_SETTINGS.W_SIGN_TEXT = StorageWidget.getItem("n_2014_w_stext","白羊座");

        var signSelectEle = WEATHER_SETTINGS.W_SET_SIGN_ELE.find("select"),
            selOption = signSelectEle.find("option:eq("+WEATHER_SETTINGS.W_SIGN_ID+")");
        selOption.select();
        WEATHER_SETTINGS.W_SET_SIGN_ELE.find("span").html(selOption.text());
        WEATHER_SETTINGS.W_SET_SIGN_ELE.addClass(WEATHER_SETTINGS.W_SIGN_CLS[selOption.val()]);
        signSelectEle.on("change",function(evt){
            var btn = $(this),
                traceKey = btn.attr("trace-key"),
                selOption = btn.find("option:selected"),
                selVal = selOption.val(),
                selText = selOption.text();
            WEATHER_SETTINGS.W_SIGN_ID = selVal;
            WEATHER_SETTINGS.W_SIGN_TEXT = selText;
            btn.parent().attr("class","opt-select "+WEATHER_SETTINGS.W_SIGN_CLS[selVal]);
            StorageWidget.setItem("n_2014_w_signid",WEATHER_SETTINGS.W_SIGN_ID);
            StorageWidget.setItem("n_2014_w_stext",WEATHER_SETTINGS.W_SIGN_TEXT);
            // console.dir("loadSign="+WEATHER_SETTINGS.W_SIGN_TEXT);
            loadSign(WEATHER_SETTINGS.W_SIGN_TEXT);
            //添加日志
            TraceWidget.addTrace(selText,"javascript:;",traceKey);
        });

       
        /*===================添加绑定事件=======================*/
        var cityChangeBtn =  $(".weather-title span"),
            weatherReloadBtn = $(".weather-reload");
        //切换城市
        cityChangeBtn.on("click",function(evt){
            evt.stopPropagation();
            var btn = $(this);   
            WEATHER_SETTINGS.W_INIT_FLAG = true;             //重新获取设置列表 
            btn.hide();
            showWeatherSet();
        });

        //天气重试-重新加载当前cityid的数据
        weatherReloadBtn.on("click",function(evt){
            evt.stopPropagation();
            loadWeather(WEATHER_SETTINGS.W_CONFIRM_CID,WEATHER_SETTINGS.W_CONFIRM_CNAME);
            evt.preventDefault();
        });

        if(WEATHER_SETTINGS.W_SET_ELE.length){
            //天气-加载城市信息
            WEATHER_SETTINGS.W_SET_ELE.find(".prov-select select").on("change",function(evt){
                evt.stopPropagation();
                var btn = $(this),
                    btnVal = $.trim(btn.val());     
                var defaultCity = loadLocList(btnVal,"city",true);
                loadLocList(defaultCity.id,"area",true);
            });

            //天气-加载地区信息
            WEATHER_SETTINGS.W_SET_ELE.find(".city-select select").on("change",function(evt){
                evt.stopPropagation();
                var btn = $(this),
                    btnVal = $.trim(btn.val());     
                loadLocList(btnVal,"area",true);
                
            });

            //天气-确定
            WEATHER_SETTINGS.W_SET_ELE.find(".btn-confirm").on("click",function(evt){
                evt.stopPropagation();
                var areaSelect = WEATHER_SETTINGS.W_SET_ELE.find(".moz-select:eq(2)").find("select").children(":selected");
                if(areaSelect.length){
                    var curid = areaSelect.val(),
                        curText = $.trim(areaSelect.text());
                    WEATHER_SETTINGS.W_CONFIRM_CID = curid; 
                    WEATHER_SETTINGS.W_CONFIRM_CNAME = curText; 
                    WEATHER_SETTINGS.W_WRAP_ELE.find(".weather-detail").show();
                    curText && WEATHER_SETTINGS.W_WRAP_ELE.find(".weather-title b").html(curText);
                    loadWeather(curid,curText);
                }
                WEATHER_SETTINGS.W_SET_ELE.hide();
                cityChangeBtn.show();
            });

             //天气-取消
            WEATHER_SETTINGS.W_SET_ELE.find(".btn-cancel").on("click",function(evt){
                evt.stopPropagation();
                WEATHER_SETTINGS.W_SET_ELE.hide();
                cityChangeBtn.show();
                WEATHER_SETTINGS.W_SET_ELE.find(".moz-select select").empty();        
            });
        }
    };
     ...
     *   初始化星座
     */
    WeatherWidget.initSign = function(){
       // initSign();
       loadSign(WEATHER_SETTINGS.W_SIGN_TEXT);
    }
    function initSign(){
        if(WEATHER_SETTINGS.W_SIGN_INIT_FLAG) return;
        $.ajax({
               url: WEATHER_lINKS.SIGN_FETCH_LINK,
               type: "GET",
               dataType:"jsonp",
               jsonpCallback:"W_GetSignInfo",
               timeout: MOZ_INFO.TIME_OUT,
               error:function(){
                  //已有当前位置
                  WEATHER_SETTINGS.W_SIGN_INIT_FLAG = false;
                  WEATHER_SETTINGS.W_WRAP_SIGN_ELE.find(".sign-content").html(MOZ_INFO.LOAD_ERROR);
               },
               success:function(){
                  WEATHER_SETTINGS.W_SIGN_INIT_FLAG = true;
               }
          });
    }
    ...
    /*
     *   加载天气  jsonP格式调用 window.W_SaveWeather
     */
    function loadWeather(code,cname){
        //if(!code || !cname) return;
        var loadFlag = true, //是否重新请求天气数据
            cid = code,
            titleEle = WEATHER_SETTINGS.W_WRAP_ELE.find(".weather-title"),
            titlePlaceEle = titleEle.find("b"),
            detailEle = WEATHER_SETTINGS.W_WRAP_ELE.find(".weather-detail");
           
        //已经保存过（比较时间，超过间隔时间则重新获取）
        if(WEATHER_SETTINGS.W_SAVE_INFO && WEATHER_SETTINGS.W_SAVE_INFO.detail && WEATHER_SETTINGS.W_SAVE_INFO.detail.forecast 
              && WEATHER_SETTINGS.W_SAVE_INFO.cityid && code == WEATHER_SETTINGS.W_SAVE_INFO.cityid){
             var curTime = new Date().getTime(),
                 saveTime = WEATHER_SETTINGS.W_SAVE_INFO.wtime,
                 minTime = Math.abs(curTime-saveTime);
             if(minTime < WEATHER_SETTINGS.W_CHECK_INTERVAL){
                  loadFlag = false;
             }
        }
        ...
        //重新加载
        if(loadFlag){
            detailEle.find(".weather-detail-info .weather-info").html(MOZ_INFO.WEATHER_LOAD_STR);
            detailEle.find(".weather-detail-info").show().siblings().hide();
            $.ajax({
                url:"/do",
                data:{checkweatherdata:"yes",citycode:code},
                type:"post",
                dataType:"json",
                success:function(res){
                    if(res.file){
                        $.ajax({
                            ...
                            dataType:"json",
                            success:function(data){
                                saveWeather(data,code,cname);
                                writeWeather(data,cname);
                            }
                        })
                    }else{
        ...
        //从本地缓存中获取
            ...
            WEATHER_SETTINGS.W_SAVE_INFO.cityname && titlePlaceEle.html(WEATHER_SETTINGS.W_SAVE_INFO.cityname);  //设置title
            writeWeather(WEATHER_SETTINGS.W_SAVE_INFO.detail,cname);
        }        
    }
    ...
    function loadProvince(){
           $.ajax({
                   url:WEATHER_lINKS.PROV_FETCH_LINK,
                   type: "GET",
                   dataType:"jsonp",
                   jsonpCallback:"W_GetLocaleInfo",
                   timeout: MOZ_INFO.TIME_OUT,
                   error:function(){
                      fetchCityError();
                   },
    ...
    function loadLocList(code,ctype,selectFlag,selectId){
        var list,
            htm,
            selEle,
            res = null;
        switch(ctype){
            case "prov":
                    list = WEATHER_LOC_INFO.PROV_LIST;
                    selEle = WEATHER_SETTINGS.W_SET_ELE.find(".prov-select select");
                                   
                    break;
            case "city":
                   
                    list = (WEATHER_LOC_INFO.PROV_LIST)[code]["children"];
                    selEle = WEATHER_SETTINGS.W_SET_ELE.find(".city-select select");
                                   
                    break;
            case "area":
                    list = (WEATHER_LOC_INFO.CITY_LIST)[code]["children"];
                    selEle = WEATHER_SETTINGS.W_SET_ELE.find(".area-select select");
                    break;
        }
        var index = 0;
       ...
       if(list){
	            ...
	            (index == 0) && (res = temp);
	            index++;
	            htm += "<option value='"+temp.id+"'>"+temp.name+"</option>"; 
	        });
       }
    ...
    /*
     *  显示天气设置infoFlag true时表示加载地区信息失败（需要显示错误信息，同时隐藏定位切换）
     */
    function showWeatherSet(infoFlag){
        var errEle = WEATHER_SETTINGS.W_SET_ELE.find(".weather-set-error");
        WEATHER_SETTINGS.W_SET_ELE.show();
        setLocale(infoFlag);

        //初始加载（自动定位失败，需要手动定位）
        if(infoFlag){
           WEATHER_SETTINGS.W_SET_ELE.find(".btn-cancel").hide();
           errEle.show();  
        }else{
           WEATHER_SETTINGS.W_SET_ELE.find(".btn-cancel").show();
           errEle.hide();
        }
    }
     ...
     */
    function setLocale(){
        var provSelect = WEATHER_SETTINGS.W_SET_ELE.find(".moz-select:eq(0)"),
            pSelEle = provSelect.find("select");
        //若省为空则为第一次加载
        if(pSelEle.is(":empty")){
            WEATHER_SETTINGS.W_INIT_FLAG = true;  //重新获取设置列表
            loadProvince();
        }else{
            WEATHER_SETTINGS.W_INIT_FLAG = false;  //无需重新获取设置列表
        }
    }

    function loadSign(scode){
    	// console.dir("loadSign="+scode);

       //从内存中获取信息
       //if(WEATHER_SETTINGS.W_SIGN_INIT_FLAG){

            var index = scode ? scode : "",
                list = WEATHER_SETTINGS.W_SIGN_LIST[index],
                contEle = WEATHER_SETTINGS.W_WRAP_SIGN_ELE.find(".sign-content");
            contEle.html(MOZ_INFO.LOAD_STR);
	        $.ajax({
	               url: WEATHER_lINKS.SIGN_FETCH_LINK+"?sharetoken=10002&xinzuo="+index+"&jsonp=W_GetSignInfo",
	               type: "GET",
	               dataType:"jsonp",
	               jsonpCallback:"W_GetSignInfo",
	               timeout: MOZ_INFO.TIME_OUT,
	               error:function(){
	                  //已有当前位置
	                 // WEATHER_SETTINGS.W_SIGN_INIT_FLAG = false;
	                  contEle.html(MOZ_INFO.LOAD_ERROR);
	               },
	               success:function(res){
	                  //WEATHER_SETTINGS.W_SIGN_INIT_FLAG = true;
	                  if(res && res.result){
	                  	    //输出星座信息
		            	    WEATHER_SETTINGS.W_WRAP_SIGN_ELE.is(":hidden") && WEATHER_SETTINGS.W_WRAP_SIGN_ELE.show();
			                writeSign(res.result);
	                  }else{
	                  	    contEle.html(MOZ_INFO.LOAD_ERROR);
	                  }
     ...
     *  输出天气
     */
    function writeWeather(data,cname){
       var weatherShowMod = WEATHER_SETTINGS.W_WRAP_ELE.find(".weather-detail"),
           titleEle = WEATHER_SETTINGS.W_WRAP_ELE.find(".weather-title");  
        if(data && data.forecast){
           ...
           var weatherHtm = "";
           if(list.length>0){
               var infoList = [{title:"今天",className:"left"},{title:"明天",className:"middle"},{title:"后天",className:"right"}],
                   todayInfo = {};
              $.each(list,function(m,k){
                  if(m<infoList.length){
                      var tempInfo = infoList[m],
                          //type、code、degree均已weather_type为参考依据
                          weatherType = k.text1 ? k.text1 : k.text2,
                          weatherCode = k.code1 ? k.code1 : k.code2,
                          weatherDegree =  k.high ? 
                                             (k.high+"℃"+(k.low ? "~"+k.low+"℃" : "")) : 
                                                  (k.low+"℃");
                      weatherHtm += '<div class="weather-mod '+tempInfo.className+'">'+
                             '<p>'+tempInfo.title+'</p>';

                      //添加链接
                      //if(data.result.url){
                      //    weatherHtm += "<a href='http://weather.sina.com.cn/"+data.result.url+"' title='点击查看详情'>";
                     // }

                      weatherHtm += '<img src="res/weather/weathericon3/'+weatherCode+'.png">'+
                             '<i>'+weatherType+'</i><span>'+weatherDegree+'</span>';
                      //if(data.result.url){
                      //    weatherHtm += "</a>";
                      //}
                      weatherHtm += '</div>';
              ...
              //温度
                  briefWeatherDegree = todayInfo.text1 ? 
                                             (todayInfo.high+"℃"+(todayInfo.low ? "~"+todayInfo.low+"℃" : "")) : 
                                                  (todayInfo.low+"℃");

              var briefHtm = '<span><b class="weather-place">'+cname+'</b>'+briefWeatherType+' '+briefWeatherDegree+'</span>';
              weatherShowMod.find(".weather-detail-info .weather-info").html(weatherHtm);
              weatherShowMod.find(".weather-detail-info").show().siblings().hide();
              weatherShowMod.show().siblings().hide();
              WEATHER_SETTINGS.W_BRIEF_ELE.html(briefHtm).parent().show();
              if(titleEle.is(":hidden")) titleEle.show();  //对于定位失败重新加载的情况
    ...
    function writeSign(data){

          var htm = MOZ_INFO.LOAD_ERROR;
          if(data){
              htm = ' <div class="sign-mod sign-analysis">';
              if(data.summary){
                  htm +=  ' <div class="mod-title"><span>今日运势解析</span></div>'+
                          '<div class="mod-cont">'+data.summary+'</div></div>'+
                          '<div class="sign-mod sign-luck">';
              }
             // if(data.detail){
                  htm +=   '<div class="mod-title"> <span>今日开运</span></div>'+
                           '<div class="mod-cont"><p>幸运数字：'+data.number+'</p>'+
                           '<p>幸运颜色：'+data.color+'</p>'+
          ...
          WEATHER_SETTINGS.W_WRAP_ELE.find(".sign-content").html(htm);
          WEATHER_SETTINGS.W_WRAP_ELE.show();
    ...
    function fetchCityError(){
          var titleEle = WEATHER_SETTINGS.W_WRAP_ELE.find(".weather-title"),
              setBubbleEle = WEATHER_SETTINGS.W_SET_ELE.find(".bubble-error");
          titleEle.find("span").show();
          if(!setBubbleEle.length){
              WEATHER_SETTINGS.W_SET_ELE.prepend("<div class='bubble-error empLink'></div>");
              setBubbleEle = WEATHER_SETTINGS.W_SET_ELE.find(".bubble-error");
          }
          setBubbleEle.html(MOZ_INFO.FETCH_CITY_FAILED).show(500,function(){
               setTimeout(function(){
                  setBubbleEle.hide();
                  WEATHER_SETTINGS.W_SET_ELE.hide();
                  WEATHER_SETTINGS.W_SET_ELE.find(".moz-select select").empty();
        ...
        /*================weather jsonp调用==============*/
    //获取天气数据
     function saveWeather(res,cid,cname){
        if(res && res.forecast && res.forecast.length){
        	WEATHER_SETTINGS.W_SAVE_INFO = {};
            WEATHER_SETTINGS.W_SAVE_INFO.wtime= new Date().getTime();
            WEATHER_SETTINGS.W_SAVE_INFO.cityid = cid;
            WEATHER_SETTINGS.W_SAVE_INFO.cityname = cname;
            WEATHER_SETTINGS.W_SAVE_INFO.detail = res;
            WEATHER_SETTINGS.W_CITY_ID = cid;
            WEATHER_SETTINGS.W_CITY_NAME = cname;
            StorageWidget.setItem("n_2014_w_info",WEATHER_SETTINGS.W_SAVE_INFO);
        }
     ...
     * 获取星座信息，并保存至内存
    */
    window.W_GetSignInfo = function(res){
        if(res){
            WEATHER_SETTINGS.W_SIGN_LIST = res;
            var info = WEATHER_SETTINGS.W_SIGN_LIST[WEATHER_SETTINGS.W_SIGN_ID];
            info && writeSign(info);
        }
    }

    /*获取地理位置*/
    window.W_GetLocaleInfo = function(data){
        var initFlag = true;
        $.each(data,function(i,n){
                            var pid = parseInt(n.parent_id);
                            if(isNaN(n.parent_id) || pid >=100){ //区县
       ...
       var  saveInfo = StorageWidget.getItem("n_2014_w_info",null),
        ...
        if(initFlag){
            ...
            if(saveInfo){
                ...
                saveInfo.cityname && (WEATHER_SETTINGS.W_CITY_NAME = saveInfo.cityname);
                WEATHER_SETTINGS.W_SAVE_INFO = saveInfo;
                //根据存储的城市  获取其所在的 城市 以及省份
                var areaInfo = WEATHER_LOC_INFO.AREA_LIST[WEATHER_SETTINGS.W_CITY_ID];
                if(areaInfo){//区县
                    var aid = areaInfo.id,
                        cid = areaInfo.parent_id,
                        cityInfo = (WEATHER_LOC_INFO.CITY_LIST)[cid],
                        pid = cityInfo.parent_id;

                }else{//城市
                    var cityInfo = WEATHER_LOC_INFO.CITY_LIST[WEATHER_SETTINGS.W_CITY_ID],
                        cid = cityInfo.id,
                        aid = cid,
                        pid = cityInfo.parent_id;
         ...
         //显示省份列表
          var defaultProv = loadLocList(null,"prov",true,pid);

          //显示默认的城市信息
          var defaultCity = loadLocList(pid? pid :defaultProv.id,"city",true,cid);
          
          //显示默认的区县信息
          var defaultArea = loadLocList(cid ? cid :defaultCity.id,"area",true,aid);
          WEATHER_SETTINGS.W_DEFAULT_ID = defaultArea["id"];
          WEATHER_SETTINGS.W_DEFAULT_NAME = defaultArea["name"];
     

      if(initFlag){//初始化第一次加载
            if(!saveInfo){
                WEATHER_SETTINGS.W_CITY_ID = WEATHER_SETTINGS.W_DEFAULT_ID;
                WEATHER_SETTINGS.W_CITY_NAME = WEATHER_SETTINGS.W_DEFAULT_NAME;
        ...
        PopWindow.prototype = {
            bindEvents: function(){
                ...
                $(".pop-win-close,.pop-win-bg").on("click",function(evt){
                    evt.stopPropagation();
                    if(opts.close && $.isFunction(opts.close)){opts.close.call(that);}else{clearPopWin();}   
                    evt.preventDefault();
                });
                $(".btn-confirm").on("click",function(evt){
                    evt.stopPropagation();
                    if(opts.success && $.isFunction(opts.success)){opts.success.call(that,this,opts.target,opts.successCallback);}else{clearPopWin();} 
                });
                $(".btn-cancel").on("click",function(evt){
                    evt.stopPropagation();
                    if(opts.cancel && $.isFunction(opts.cancel)){opts.cancel.call(that);}else{ clearPopWin();}
                });
            },
            init: function(){
                var that = this,
                ...
                var winHtm = "",
                    winCont = "",
                    bgHtm = "";
                winCont =  "  <div id='pop-title' class='pop-win-title'>"+
                           "      <span>"+opts.title+"</span><a id='pop-close' href='javascript:;' class='pop-win-close'>×</a>"+
                           "  </div>"+
                           "  <div class='pop-win-cont'>"+opts.content+"<div id='msg' class='pop-msg'></div><div class='pop-tip'></div></div>";
...
(function(){
    ...
    RecordWidget.prototype = {
        ...
        init: function(){
            ...
            if(NEW_EXTENSION || ceHome){ //兼容FF35之后的主页扩展
         ...
         deleteRecord:function(btn,type){
            ...
            var deleteId = "";
             ...
             if(type == "most" || type == "last"){
                    ...
                    btnParent.remove();
                    if(NEW_EXTENSION){
                         var detail = {  id: window.mozCNChannel,
                                      message: {
                                        id: deleteId,
                                        key: deleteKey,
                                        parameters: {
            ...
            var that      =   this,
              ...
              if(NEW_EXTENSION){ //新扩展
                ...
                if(ceHome){
                    ...
                    if(frequent && frequent.queryAsync){//异步请求
     ...
     function recordLinkTemp(results,type,filter,titName,uriName,spanAttr){
            ...
            if(results){
                ...
                for(var r in results){
                    ...
                    htm += "<li><b data-index='"+r+"'>×</b>"+
                           "<a href='" + rURIStr + "' title='" + rLinkText + "'>" + rLinkText + "</a></li>";
                    index++;
                }
                htm += "</ul>";
            }
    ...
    SiteWidget.prototype = {
        ...
        bindEvent: function(){
            ...
            $("#site-manage-reset").click(function(evt){
                evt.stopPropagation();
                var btn = $(this);
                if(!btn.hasClass("disabled")){that.getDefaultLinksNew(btn);}
                evt.preventDefault();
            });
            //保存我的网址
            $("#site-manage-save").click(function(evt){
                evt.stopPropagation();
                var btn = $(this);
                if(!btn.hasClass("disabled")){that.syncLinksToServer(btn);}
                evt.preventDefault();
            });
            //从旧版同步
            ...
            $(".sm-recommend p").hover(function(evt){
                evt.stopPropagation();
                var btn = $(this);
                if(btn.hasClass("item-added")){
                    $(".link-cancel-hint").hide();
        ...
        init:function(){
            ...
            if(customList == undefined){  //没有自定义过网址
               // customList = [];
        ...
        addLink: function(url,name,btn){
            ...
            if(btn && btn.length){
              traceKey="site_add_system";btn.addClass("item-added");
            }   
            //添加日志
                                ...
                                if(msg){
                                    ...
                                    switch(key){
                                        case "frequent.tophosts":  //查询最近次数最多的网址（从白名单中找到相应的网址信息）
                                            ...
                                            $(document).on('click','#cainixihuan',function(evt){//给"猜你喜欢"绑定事件
        ...
        show: function(){
            ...
            if(coArr && coArr.length){
                ...
                if(showHtm){
                    ...
                    $('#module-slinks').css('marginBottom','36px').append('<div id="topsitebar" class="top-site" trace-key="site_promote"><span title="点击换一换" id="cainixihuan">猜你喜欢</span>'+showHtm+'</div>');
            ...
            while(pageArr.length < that.showSize){
                 ...
                 if(that.arrOdd.length){
                       ...
                       if(randTemp && !contains(ridArr,randTemp)){
                            ...
                            if(that.arrOdd.length < that.showSize){ ridArr.push(randTemp);}  //最后一页时进行存储，以排除上一次循环最后一页剩余的网址，但不进行删除
...
(function(){
    ...
    MailWidget.prototype = {
        ...
        bindEvents: function(){
            ...
            formEle.find(".ipt").focus(function(evt){
            	 var btn = $(this),
            	     parEle = btn.parent();
...
(function(){
   ...
   SlideWidget.slides = [];  //保存记录slide，便于更改
   SlideWidget.prototype = {
        visWidth: 0,  // 容器的宽度(可见区域)
        contWidth: 0, //内容的宽度
        cPage:0,      //当前页数（加loop后index下标）
       // rPage:0,      //真实的页数（不加loop后index下标，与dot对应）
        tPage:0,      //总页数
        autoInterval: null,
        autoFlag: false,
        bindEvents: function(){
        ...
        switchBtnStatus:function(btn,type){
            ...
            if(type){
                 switch(type){
                      case 1: btn.removeAttr("opt-status");break;      
                      case -1:  btn.attr("opt-status","disabled");break;
                  }
            }else{
    ...
    SlideWidget.build = function(options){
        ...
        SlideWidget.slides.push(slideWidget);
    };
    //在宽度改变时重新初始化滑动区域
    SlideWidget.resetSlides = function(){
         var slides = SlideWidget.slides;
         $.each(slides,function(i,n){
             var slide = n,
                 ...
                 nextEle = settings.nextEle,
                 pageIndex = 1;
             slide.resetDimension();
             if(!settings.loop){
                pageIndex = 0;
                if(tPage>1 && nextEle.length){  nextEle.removeClass("disabled");}
                prevEle.length && prevEle.addClass("disabled");
             }
             slide.moveToPage(pageIndex);
         });
    }
